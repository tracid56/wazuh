...
int Read_Cluster(XML_NODE node, void *d1, __attribute__((unused)) void *d2) {
    ...
    for (i = 0; node[i]; i++) {
        if (!node[i]->element) {
            ...
            mwarn("Detected a deprecated configuration for cluster. Name option is not longer available.");
            ...
            mwarn("Detected a deprecated configuration for cluster. Key option is not longer available.");
